...
from settings_new import config as conf
...
def my_lock(mode):
    lock = os.path.join(conf['ice_box'],'lockfile')
    if mode == "create":
...
def run_java_component(jar, args, timeout=None):
    ...
    cmd = [conf['java_sdk'] + 'java', '-jar', jar, *args]
    ...
    try:
        ...
        print('Java component timed out.')
...
def resolve_sample_filename(name, radix=0, ext='apk'):
    ...
    return ('{}/' + '{}/' * radix + '{}.{}').format(
        conf['goodware_location'], *name[:radix], name, ext)
...
def perform_logging(message):
    message = str(message)
    log_path = os.path.join(conf["project_root"] , "log.txt")
...
def perform_logging_for_attack(message):
    message = str(message)
    log_path = os.path.join(conf["project_root"] , "log_attack.txt")
    if os.path.exists(log_path) == False:
         with open(log_path, 'w') as f:
             f.write(message + '\n')             
    else:
